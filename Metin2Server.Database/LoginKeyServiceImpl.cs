...
public class LoginKeyServiceImpl : LoginKeyService.LoginKeyServiceBase
{
    ...
    public override async Task<IssueLoginKeyResponse> Issue(IssueLoginKeyRequest request, ServerCallContext context)
    {
        ...
        return new IssueLoginKeyResponse
        {
            Key = entry.Key,
    ...
    public override async Task<ConsumeLoginKeyResponse> Consume(ConsumeLoginKeyRequest request, ServerCallContext context)
    {
        ...
        if (!ok || entry is null)
        {
            return new ConsumeLoginKeyResponse { Ok = false };
        }

        return new ConsumeLoginKeyResponse
        {
            Ok = true,
